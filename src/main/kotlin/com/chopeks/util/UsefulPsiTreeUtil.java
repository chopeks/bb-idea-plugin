...
public class UsefulPsiTreeUtil {
    public static ASTNode[] findChildrenRange(ASTNode[] elements, int startOffset, int endOffset) {
        ...
        int to = j;
        if (j < elements.length && !isSemicolon(elements[j])) {
            // try eat until ';'
            while (j + 1 < elements.length && (isSemicolon(elements[j + 1]) ||
    ...
    public static boolean isSemicolon(ASTNode element) {
        return element.getElementType() == SquirrelTokenTypes.SEMICOLON || element.getElementType() == SquirrelTokenTypes.SEMICOLON_SYNTHETIC;
    }
